...
void Algoritmos::vendedor(GNDGD grafo, Vertice v) {//Diccionario
    ...
    if (v2 != nodoNulo) {
        ...
        int cantSolF = 0;
        int solAct = 0;
        int cantVert = grafo.numVertices();
        solucionOp.reserve(grafo.numVertices());
        solActual.reserve(grafo.numVertices());
        visitados.agregar(v);
...
void Algoritmos::vendedorR(GNDGD grafo, Vertice v1, Vertice v2, DicV visitados, vectorV& solOp, vectorV solAc,
    ...
    cantVertF--;
    if (cantVertF == 0 && grafo.existeArista(v2, solAc[0])) {
        solAct += grafo.peso(v2, solAc[0]);
        cantSolF++;
        if (solAct < solOpt) {
            solOpt = solAct;
            solOp = solAc;
